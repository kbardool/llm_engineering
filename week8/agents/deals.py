    ...
    def fetch(cls, show_progress: bool = False) -> List[Self]:
        ...
        for feed_url in feed_iter:
            ...
            for entry in feed.entries[:10]:
                ...
                time.sleep(0.05)
            print(f"*** Finished fetching {feed_url}")
        return deals
...
class Deal(BaseModel):
    ...
    price: float = Field(
        ...
        "Be sure to give the actual price; for example, if a deal is described as $100 "
        "off the usual $300 price, you should respond with $200"
    )
...
class DealSelection(BaseModel):
    ...
    deals: List[Deal] = Field(
        ...
        " You should be confident that the price reflects the deal, that it is a good deal, with a clear description"
    )
